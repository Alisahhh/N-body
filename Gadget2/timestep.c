...
void advance_and_find_timesteps(void)
{
	...
	RDMA_Allreduce(&dispmax, &globmax, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
	RDMA_Allreduce(&disp2sum, &globdisp2sum, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	...
	RDMA_Allreduce(&ti_step, &All.PresentMinStep, 1, MPI_INT, MPI_MIN, MPI_COMM_WORLD);
	...
	if (All.PM_Ti_endstep == All.Ti_Current) /* need to do long-range kick */
	{
		...
		for (i = 0; i < NumPart; i++)
		{
			...
			if (P[i].Type == 0)
			{
				...
				for (j = 0; j < 3; j++)
					SphP[i].VelPred[j] = P[i].Vel[j] + P[i].GravAccel[j] * dt_gravkickA + SphP[i].HydroAccel[j] * dt_hydrokick + P[i].GravPM[j] * dt_gravkickB;
			}
		}
	}
#endif
...
)
{
   ...
   */
	...
	if (dt < All.MinSizeTimestep)
	{
		...
		if (P[p].Type == 0)
		{
			printf("Part-ID=%d  dt=%g dtc=%g ac=%g xyz=(%g|%g|%g)  hsml=%g  maxsignalvel=%g dt0=%g eps=%g\n",
...
void find_dt_displacement_constraint(double hfac /*!<  should be  a^2*H(a)  */)
{
	...
	if (All.ComovingIntegrationOn)
	{
		...
		RDMA_Allreduce(v, v_sum, 6, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		RDMA_Allreduce(mim, min_mass, 6, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
