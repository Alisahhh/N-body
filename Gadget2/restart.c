...
void restart(int modus)
{
	...
	for (groupTask = 0; groupTask < nprocgroup; groupTask++)
	{
		if (ThisTask == (masterTask + groupTask)) /* ok, it's this processor's turn */
		{
			...
			{
				RDMA_MakeAll(&all_task0, sizeof(struct global_data_all_processes), 0, MPI_COMM_WORLD);
			}
			...
			if (N_gas > 0)
			{
				...
				{
					printf("SPH: it seems you have reduced(!) 'PartAllocFactor' below the value of %g needed to load the restart file.\n",
			...
			{
				printf("Tree storage: it seems you have reduced(!) 'PartAllocFactor' below the value needed to load the restart file (task=%d). "
