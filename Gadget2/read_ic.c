...
#define SKIP2                                    \
	...
	if (ThisTask == readTask)
	{
		...
		RDMA_Recv(&header, sizeof(header), readTask, MPI_COMM_WORLD);
						...
						for (task = readTask; task <= lastTask; task++)
						{
							...
							do
							{
								...
								if (ThisTask != readTask && task == ThisTask)
									RDMA_Recv(CommBuffer, bytes_per_blockelement * pc, readTask, MPI_COMM_WORLD);
...
int find_files(char *fname)
{
	...
	RDMA_MakeAll(&header, sizeof(header), 0, MPI_COMM_WORLD);
