    ...
    if ((rep == 1 && ThisTask < partner) || (rep == 0 && ThisTask > partner))
    {
      if (recv_count > 0)
      {
        if (sphflag)
        {
          ...
          RDMA_Recv(&P[N_gas], recv_count * sizeof(struct particle_data), partner, MPI_COMM_WORLD);
          RDMA_Recv(&Key[N_gas], recv_count * sizeof(peanokey), partner, MPI_COMM_WORLD);
          RDMA_Recv(&SphP[N_gas], recv_count * sizeof(struct sph_particle_data), partner, MPI_COMM_WORLD);
          N_gas += recv_count;
        }
        else
        {
          RMDA_Recv(&P[NumPart], recv_count * sizeof(struct particle_data), partner, MPI_COMM_WORLD);
          RMDA_Recv(&Key[NumPart], recv_count * sizeof(peanokey), partner, MPI_COMM_WORLD);
        }
...
void domain_sumCost(void)
{
  ...
  RDMA_Allreduce(local_DomainWork, DomainWork, NTopleaves, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  RDMA_Allreduce(local_DomainCount, DomainCount, NTopleaves, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  RMDA_Allreduce(local_DomainCountSph, DomainCountSph, NTopleaves, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
void domain_findExtent(void)
{
  ...
  RDMA_Allreduce(xmin, xmin_glob, 3, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
  RDMA_Allreduce(xmax, xmax_glob, 3, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
