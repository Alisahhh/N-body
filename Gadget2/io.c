...
#define SKIP                                 \
  ...
  if (ThisTask == writeTask)
  {
    ...
    {
      RDMA_Recv(&nn[0], 6, task, MPI_COMM_WORLD);
      for (n = 0; n < 6; n++)
        ntot_type[n] += nn[n];
    }

    for (task = writeTask + 1; task <= lastTask; task++)
      RDMA_Send(&ntot_type[0], 6, task, MPI_COMM_WORLD);
  }
  else
  {
    RMDA_Send(&n_type[0], 6, writeTask, MPI_COMM_WORLD);
    RMDA_Recv(&ntot_type[0], 6, writeTask, MPI_COMM_WORLD);
  }
            ...
            for (task = writeTask, offset = 0; task <= lastTask; task++)
            {
              if (task == ThisTask)
              {
                ...
                for (p = writeTask; p <= lastTask; p++)
                  if (p != ThisTask)
                    RDMA_Send(&n_for_this_task, 1, p, MPI_COMM_WORLD);
              }
              else
                RDMA_Recv(&n_for_this_task, 1, task, MPI_COMM_WORLD);
              ...
              while (n_for_this_task > 0)
              {
                ...
                if (ThisTask == writeTask && task != writeTask)
                  RDMA_Recv(CommBuffer, bytes_per_blockelement * pc, task, MPI_COMM_WORLD);
